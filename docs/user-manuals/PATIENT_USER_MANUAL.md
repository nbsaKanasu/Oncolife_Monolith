# OncoLife Patient App - User Manual

<div align="center">

## Your Personal Health Companion During Cancer Treatment

**Version 1.0 | January 2026**

</div>

---

## Welcome to OncoLife

OncoLife is your personal health companion designed to help you track symptoms, communicate with your care team, and stay informed during your cancer treatment journey.

**Important:** OncoLife is a support tool. It does NOT replace your care team or emergency services. If you experience severe symptoms, always contact your doctor or call 911.

---

## Table of Contents

1. [Getting Started](#1-getting-started)
2. [Home Screen Overview](#2-home-screen-overview)
3. [Daily Symptom Check-In](#3-daily-symptom-check-in)
4. [Understanding Your Results](#4-understanding-your-results)
5. [My Diary](#5-my-diary)
6. [Questions for My Doctor](#6-questions-for-my-doctor)
7. [My Treatment Calendar](#7-my-treatment-calendar)
8. [Education & Resources](#8-education--resources)
9. [My Profile & Settings](#9-my-profile--settings)
10. [Getting Help](#10-getting-help)

---

## 1. Getting Started

### How You Got Here

Your oncology clinic enrolled you in OncoLife. You should have received:
- A **welcome email** or **text message**
- Your **username** (your email address)
- A **temporary password**

### First-Time Login

**Step 1: Open the App/Website**
- Go to: **app.oncolife.com**
- Or download the OncoLife app from the App Store/Google Play

**Step 2: Enter Your Credentials**
- Enter the email address from your welcome message
- Enter the temporary password
- Tap **"Login"**

**Step 3: Create Your New Password**
```
Your password must have:
✓ At least 8 characters
✓ One uppercase letter (A-Z)
✓ One lowercase letter (a-z)
✓ One number (0-9)
✓ One special character (!@#$%^&*)
```
- Enter your new password
- Confirm your new password
- Tap **"Update Password"**

**Step 4: Read & Accept the Acknowledgement**
- Read the acknowledgement text carefully
- Check the box: "I understand this tool does not replace my care team"
- Tap **"Continue"**

**Step 5: Accept Terms & Privacy**
- Review the Terms of Service
- Review the Privacy Policy
- Check both acceptance boxes
- Tap **"Accept & Continue"**

**Step 6: Set Your Reminder Preferences**
- Verify your phone number and email
- Choose how you want to receive reminders:
  - 📧 **Email** - Daily reminder email
  - 📱 **Text Message (SMS)** - Daily text reminder
- Choose your preferred reminder time
- Tap **"Save & Continue"**

🎉 **You're all set!** You'll now see your home screen.

---

## 2. Home Screen Overview

When you open OncoLife, you'll see your home screen with these main sections:

```
┌─────────────────────────────────────┐
│  👋 Good Morning, [Your Name]       │
│                                     │
│  ┌─────────────────────────────┐   │
│  │  🩺 Start Daily Check-In    │   │
│  │  How are you feeling today? │   │
│  └─────────────────────────────┘   │
│                                     │
│  📅 Upcoming                        │
│  • Chemo: Jan 15, 2026             │
│                                     │
│  📊 Recent Activity                 │
│  • Jan 4: Check-in completed ✓     │
│  • Jan 3: Check-in completed ✓     │
│                                     │
│  ─────────────────────────────────  │
│  🏠  📓  ❓  💊  👤                │
│  Home Diary Questions Meds Profile  │
└─────────────────────────────────────┘
```

### Navigation Bar Icons

**On Mobile (Bottom Bar):**

| Icon | Name | What It Does |
|------|------|--------------|
| 💬 | Symptom Check | Start your daily symptom check-in |
| 📄 | Summaries | View past triage summaries |
| 📓 | Diary | Your personal health diary |
| ❓ | Questions | Questions for your doctor |
| 📚 | Education | Health learning resources |

**On Desktop/Tablet (Sidebar):**

| Icon | Name | What It Does |
|------|------|--------------|
| 💬 | Symptom Check | Start your daily symptom check-in |
| 📄 | Summaries | View past triage summaries |
| 📓 | Diary | Your personal health diary |
| ❓ | Questions | Questions for your doctor |
| 📚 | Education | Health learning resources |
| 👤 | Profile | Your account settings |
| 🌙 | Dark Mode | Switch between light and dark themes |

---

## 3. Daily Symptom Check-In

**We recommend completing a daily check-in** to help track your symptoms over time.

### Starting Your Check-In

1. From the home screen, tap **"Start Daily Check-In"**
2. You'll begin the guided 7-step symptom assessment

### Step 1: Medical Disclaimer

First, you'll see an important disclaimer:

```
📋 IMPORTANT MEDICAL DISCLAIMER

This system is an automated symptom checker. It is NOT 
a substitute for professional medical advice, diagnosis, 
or treatment.

If you believe you are having a medical emergency, 
call 911 immediately.
```

✅ Tap **"I Understand - Start Triage"** to continue.

### Step 2: Quick Check-In (Treatment Timeline)

**Ruby needs a couple of important details to help your care team:**

**Question 1: Last Chemotherapy**
```
📅 When was your last chemotherapy session?
```
Options: Today, Yesterday, 2-3 days ago, 4-7 days ago, 1-2 weeks ago, More than 2 weeks ago, or "I haven't had chemotherapy yet"

**Question 2: Next Physician Visit**
```
📅 When is your next scheduled physician visit?
```
Options: Today, Tomorrow, In 2-3 days, This week, Next week, In 2+ weeks, or "Not scheduled yet"

**Why does this matter?** This information helps your care team understand your treatment timeline and prioritize any concerns.

### Step 3: Emergency Safety Check

**This is a quick safety check for urgent symptoms.**

You'll see a list of emergency symptoms:
- ⚠️ Trouble breathing
- ⚠️ Chest pain
- ⚠️ Uncontrolled bleeding / Blood in stool or urine
- ⚠️ Fainting
- ⚠️ Confusion

**If you have ANY of these:** Select them and get immediate guidance.

**If you don't have any:** Tap **"None of these - Continue"**

### Step 4: Select Your Symptoms

Symptoms are organized into helpful groups:

| **Digestive Health** | **Pain & Nerve** |
|---------------------|------------------|
| 🤢 Nausea | 😣 Pain |
| 🤮 Vomiting | 🦶 Numbness/Tingling |
| 🚽 Diarrhea | |
| 😣 Constipation | |
| 🍽️ No Appetite | |
| 👄 Mouth Sores | |

| **Systemic & Infection** | **Skin & External** |
|-------------------------|-------------------|
| 🤒 Fever | 🔴 Skin Rash |
| 🩸 Bleeding/Bruising | 💧 Swelling |
| 😴 Fatigue | 👁️ Eye Complaints |
| 🫁 Cough | |
| 🚽 Urinary Problems | |

**Tap all symptoms that apply**, then tap **"Continue"**

**If you feel fine:** Tap "I feel okay today" for a quick check-in.

### Step 5: Chat with Ruby 💎

Ruby is your friendly health assistant! She'll ask you questions about each symptom you selected.

```
💎 Ruby: "Hello! I am Ruby, your automated triage assistant.
         I'm here to help assess your symptoms."

💎 Ruby: "Do you currently have a fever?"

         [Yes]  [No]

    You: "Yes"

💎 Ruby: "What is your temperature?"

         [Less than 100°F]  [100-101°F]  
         [101-103°F]  [Over 103°F]
```

**Tips for Answering:**
- Answer honestly for the best guidance
- If you're unsure, select the closest option
- Your answers appear on the right side (purple bubbles)
- Ruby's questions appear on the left

### Step 6: View Your Results

After answering all questions, you'll see your triage result (see next section).

### Step 7: Summary & Actions

At the end, you'll see a complete summary with options:

| Button | What It Does |
|--------|--------------|
| 📥 **Download** | Save your summary as a PDF |
| 📓 **Save to Diary** | Add this check-in to your diary |
| 🔄 **Report Another** | Check another symptom |
| ✅ **Done** | Finish and return home |

---

## 4. Understanding Your Results

After your check-in, OncoLife analyzes your symptoms and provides guidance:

### Result Types

#### 🟢 Green - Everything Looks Good
```
"Your symptoms are mild. Continue to monitor how you feel."

✓ No immediate action needed
✓ Continue your daily check-ins
✓ Review your education materials
```

#### 🟡 Yellow - Monitor Closely
```
"Some of your symptoms need attention. Monitor closely."

⚠️ Track these symptoms over the next 24 hours
⚠️ If symptoms worsen, contact your care team
📞 Call your clinic if no improvement
```

#### 🟠 Orange - Contact Care Team
```
"Please contact your care team today."

📞 Call your clinic or nurse navigator
📋 Share your symptom summary with them
⏰ Don't wait - reach out today
```

#### 🔴 Red - Seek Immediate Help
```
⚠️ URGENT: Seek medical attention immediately

🚨 Call 911 or go to the nearest ER
📞 Your care team has been notified
❌ Do not wait - this needs immediate attention
```

### Education Content

After each check-in, you'll receive:
- **Symptom Tips:** Simple steps to manage your symptoms
- **Read More Links:** PDFs with detailed information
- **Care Team Handout:** Always available for reference

**Important Reminder:** At the bottom of every result, you'll see:
> "This information is for educational purposes only. Always follow your care team's instructions."

---

## 5. My Diary

Your diary is your personal health journal.

### What's in My Diary?

1. **Auto-Generated Entries** - Created after each check-in
2. **Your Personal Notes** - Entries you write yourself

### Viewing Your Diary

1. Tap the **📓 Diary** icon
2. See entries organized by date
3. Tap any entry to view details

### Creating a Diary Entry

1. Tap **"+ Add Entry"**
2. Enter a title (optional, e.g., "Feeling better today")
3. Write your notes:
   ```
   Example:
   "Nausea was less today. Was able to eat breakfast 
   and lunch. Energy still low in afternoon. Took a 
   2-hour nap."
   ```
4. **Share with Doctor:** Toggle ON if you want your doctor to see this
5. Tap **"Save"**

### What Your Doctor Sees

| You See | Doctor Sees |
|---------|-------------|
| All your entries | Only "Shared" entries |
| Private notes | ❌ Never sees these |
| Auto-summaries | ✅ Yes |

**Tip:** Use "Share with Doctor" for things you want to discuss at your appointment.

---

## 6. Questions for My Doctor

Forgot to ask something during your last visit? Write it down here!

### Creating a Question

1. Tap the **❓ Questions** icon
2. Tap **"+ Add Question"**
3. Write your question:
   ```
   Example:
   "Should I take my anti-nausea medication before 
   or after eating?"
   ```
4. Choose a category:
   - 🩺 **Symptom** - About your symptoms
   - 💊 **Medication** - About your medicines
   - 📋 **Treatment** - About your treatment plan
   - 📌 **Other** - Anything else
5. **Share with Doctor:** Toggle ON to let your doctor see it
6. Tap **"Save"**

### Managing Your Questions

| Action | How To |
|--------|--------|
| Share/Unshare | Tap the share toggle |
| Mark as Answered | Tap ✓ after your doctor responds |
| Delete | Swipe left and tap "Delete" |

### When Will My Doctor See It?

- Your doctor can see shared questions in their dashboard
- Questions may also appear in their weekly report
- Discuss at your next appointment or call

---

## 7. My Treatment Calendar

Track your chemotherapy appointments and treatment schedule. The treatment calendar is now integrated into your **Profile** page for easier access.

### Viewing Your Treatment Timeline

1. Go to **👤 Profile** page
2. Scroll down to the **"Chemotherapy Timeline"** section
3. See your upcoming and past treatments:
   - **📆 Upcoming Sessions** - Highlighted in blue with countdown
   - **📋 Recent Sessions** - Shown in gray for reference

### Adding a Treatment Date

1. On your Profile page, find the Timeline section
2. Click **"+ Add Date"** button
3. Select the date from the calendar picker
4. Click **"✓ Add Date"** to save

### Managing Dates

| Action | How To |
|--------|--------|
| **Add Date** | Click "+ Add Date", select date, confirm |
| **Delete Date** | Click 🗑️ icon, confirm deletion |
| **View Details** | See countdown (e.g., "In 3 days") for each date |

### Visual Timeline

The timeline shows your chemotherapy journey:

```
📆 UPCOMING SESSIONS
   ● Wed, Jan 15, 2026  [In 2 days]
   ● Wed, Jan 29, 2026  [In 16 days]

📋 RECENT SESSIONS
   ○ Wed, Jan 1, 2026   [12 days ago]
   ○ Wed, Dec 18, 2025  [26 days ago]
```

💡 **Tip:** Your profile also stores your **Last Chemotherapy Date** and **Next Physician Visit** separately in the Treatment Information section for quick reference.

---

## 8. Education & Resources

Learn more about managing your symptoms.

### Accessing Education

1. From your home screen or results, tap **"Education"**
2. Browse by category:
   - **My Current Symptoms** - Based on your check-ins
   - **Common Side Effects** - General chemo symptoms
   - **Care Team Handouts** - Important documents

### Types of Content

| Type | Description |
|------|-------------|
| 📝 **Quick Tips** | 4-6 bullet points, easy to read |
| 📄 **PDF Guides** | Detailed information to download |
| 📋 **Handouts** | Documents from your care team |

### Searching

Use the search bar to find specific topics:
- "nausea"
- "fatigue"
- "mouth sores"

---

## 9. My Profile & Settings

### Viewing Your Profile

1. Tap the **👤 Profile** icon
2. See your information organized in sections:

**Personal Information:**
- Name, Email, Phone Number
- Date of Birth
- Reminder Time

**Treatment Information:**
- Diagnosis (Cancer Type)
- Treatment Type
- Last Chemotherapy Date
- Next Physician Visit
- Doctor Name, Clinic Name

**Emergency Contact:**
- Contact Name
- Contact Phone

### Updating Your Treatment Information

Your profile now stores important treatment information that was previously asked during each symptom check-in:

1. Go to Profile → **"Edit"** button
2. Update your treatment information:
   - **Last Chemotherapy Date** - When was your most recent chemo session
   - **Next Physician Visit** - Your upcoming appointment with your doctor
   - **Diagnosis** - Your cancer type/diagnosis
   - **Treatment Type** - e.g., Chemotherapy, Immunotherapy
3. Tap **"Save Changes"**

💡 **Tip:** Keeping this information up-to-date helps your care team provide better support.

### Chemotherapy Timeline

Below your profile information, you'll see a **Chemotherapy Timeline** showing:

| Section | What It Shows |
|---------|---------------|
| 📆 **Upcoming Sessions** | Scheduled chemo dates with countdown |
| 📋 **Recent Sessions** | Your past chemo dates for reference |

**To Add a Chemo Date:**
1. Click **"+ Add Date"**
2. Select the date from the calendar
3. Click **"Add Date"** to confirm

**To Remove a Date:**
1. Click the 🗑️ icon next to any date
2. Confirm deletion

### Changing Reminder Settings

1. Go to Profile → **"Reminder Settings"**
2. Update your preferences:
   - Reminder Method (Email or SMS)
   - Reminder Time
3. Tap **"Save"**

### Updating Your Contact Info

1. Go to Profile → **"Edit"** button
2. Update your phone, emergency contact, or other details
3. Tap **"Save Changes"**

### Logging Out

1. Go to Profile
2. Tap **"Log Out"**
3. Confirm

### Using Dark Mode 🌙

OncoLife supports a dark theme that's easier on your eyes, especially at night.

**To Turn On Dark Mode:**

**On Phone:**
1. Look for the 🌙/☀️ icon in the top header
2. Tap it to switch between light and dark

**On Computer/Tablet:**
1. Look in the sidebar (left menu)
2. Find "Light Mode" or "Dark Mode" at the bottom
3. Tap the toggle to switch

**Automatic Mode:**
- If your phone/computer is set to dark mode, OncoLife will follow that setting automatically!

| Light Mode | Dark Mode |
|------------|-----------|
| White backgrounds | Dark navy backgrounds |
| Teal accent colors | Lighter teal accents |
| Best for daytime | Easier on eyes at night |

---

## 10. Getting Help

### Emergency Situations

**If you have a medical emergency:**
```
🚨 Call 911 immediately

Do NOT use OncoLife for emergencies.
Go to the nearest emergency room.
```

### Technical Support

**Having trouble with the app?**
- Email: support@oncolife.com
- Phone: 1-800-XXX-XXXX
- In-app: Profile → "Get Help"

### Frequently Asked Questions

**Q: How often should I do a check-in?**
A: We recommend daily check-ins, ideally at the same time each day.

**Q: What if I miss a day?**
A: That's okay! Just complete your check-in when you can.

**Q: Can my family see my information?**
A: No. Only you and your care team can see your data.

**Q: Is my information private?**
A: Yes. OncoLife is HIPAA-compliant and your data is encrypted.

**Q: What if I feel fine?**
A: You can still check in! Tap "I feel okay today" for a quick check.

**Q: Who sees my diary entries?**
A: Only entries you mark "Share with Doctor" are visible to your care team.

---

## Quick Reference Card

### Daily Routine
```
☐ Open OncoLife
☐ Tap "Start Daily Check-In"
☐ Select any symptoms
☐ Answer questions honestly
☐ Review your results
☐ Read education tips
```

### When to Contact Your Care Team
- 🟠 Orange result
- Symptoms worsening
- New or unusual symptoms
- Questions about medications

### When to Call 911
- 🔴 Red result
- Severe breathing problems
- Chest pain
- High fever with chills
- Severe bleeding
- Confusion

---

## Notes

Use this space to write down important information:

```
My Care Team:

Doctor: _______________________
Phone: ________________________

Nurse Navigator: _______________
Phone: ________________________

Pharmacy: _____________________
Phone: ________________________

My Next Appointment: ___________
```

---

<div align="center">

**Thank you for using OncoLife!**

*Your health journey matters to us.*

Questions? Contact support@oncolife.com

</div>

---

*Version 2.0 | January 2026 - Updated for new Ruby chat experience and dark mode*
*© 2026 OncoLife Health Technologies*

